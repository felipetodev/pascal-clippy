...
export default function Home () {
  ...
  const handleAnswer = async () => {
    ...
    try {
      fetchEventSource('/api/vector', {
        ...
        onmessage: (event) => {
          if (event.data === '[DONE]') {
            ...
            const data = JSON.parse(event.data) as { data: string, text: string }
            if (data.text) {
      ...
      return toast.error('Ocurrió un error al procesar tu pregunta, intenta de nuevo')
    } finally {
      scrollToBios()
    }
  }
      ...
      <div className="flex flex-1 w-full flex-col items-center justify-center text-center px-4 mt-12 sm:mt-20">
        <div className='flex flex-col lg:flex-row gap-5 items-center'>
          <h1 className="sm:text-6xl text-4xl max-w-[750px] font-bold text-blue-400">
            Instituto Pascal Assistant v2.0
          </h1>
        </div>
